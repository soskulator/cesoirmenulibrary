...
export function BottomNav() {
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-background/95 backdrop-blur-md border-t border-border h-16 md:h-20 flex items-center justify-around px-2 md:px-8" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {navItems.map((item) => {
          if (item.path === '__search__') {
            return (
              <button
                ...
                className="flex flex-col items-center gap-1 py-2 px-3 min-w-[52px] md:min-w-[72px] md:py-3 text-muted-foreground active:text-copper transition-colors"
              >
          ...
          return (
            <Link
              ...
              to={item.path}
              className={cn(
                'flex flex-col items-center gap-1 py-2 px-3 min-w-[52px] md:min-w-[72px] md:py-3',
