  ...
  try {
    ...
    const roleName =
      role === "lead_admin"
        ...
        : role === "server"
        ? "Server"
        : role === "server_assistant"
        ? "Server Assistant"
        : "Team Member";

    const inviterText = invitedByName
      ? ` by <strong style="color: #2C241E; font-family: Georgia, 'Times New Roman', serif;">${escapeHtml(invitedByName)}</strong>`
      : "";

    console.log(
      `Sending invitation email to ${email} for role ${role} (requested by admin ${userId})`
    );
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "You're Invited to Join the Ce Soir Staff Portal",
        html: buildEmailHtml({
          preheader: "You've been invited to join the Ce Soir staff training portal.",
          recipientName: "",
          headline: "You're Invited",
          body: `
            <p style="color: #4A3728; line-height: 1.8; margin: 0 0 20px; font-size: 15px; font-family: Georgia, 'Times New Roman', serif; text-align: center;">
              You have been invited${inviterText} to join the Ce Soir staff training portal as a <strong style="color: #2C241E; font-family: Georgia, 'Times New Roman', serif;">${escapeHtml(roleName)}</strong>.
            </p>
          ...
          footnote: "This invitation link will expire in 7 days. If you were not expecting this, please disregard.",
        }),
      }),
    });
...
function buildEmailHtml(opts: EmailOptions): string {
  // CTA button — flat solid color only (no gradients = Outlook-safe)
  const cta =
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" class="email-container" style="max-width: 600px; width: 100%;">
          ...
          <tr>
            <td class="email-header-pad" style="background-color: #1C1410; padding: 40px 48px 32px; text-align: center;">
              ...
              <!-- Wordmark (text fallback — no image dependency) -->
          ...
          <!-- Copper accent bar (solid, no gradient = Outlook-safe) -->
          <tr>
          ...
          <tr>
            <td class="email-body-pad" style="background-color: #FAF8F4; padding: 44px 48px 32px;">
              ...
              <!-- Ornamental divider above headline -->
