...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Authenticated user ${userId} requesting test completion notification`
    );
    ...
    console.log(
      `Notifying lead admins about test completion: ${employeeName} (${testType})`
    );
    ...
    if (adminEmails.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "No lead admin emails found",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      `Sending notification to ${adminEmails.length} lead admin(s)`
    );
    ...
    const passColor = percentage >= 70 ? "#2d8a4e" : "#c43e3e";
    const passStatus =
      ...
      html: buildEmailHtml({
        ...
        body: `
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" style="width: 100%; background: #FFFFFF; border: 1px solid #DDD4C8;">
            <tr>
              <td colspan="2" style="background-color: ${percentage >= 70 ? "#f0faf4" : "#fdf4f4"}; padding: 20px 24px; text-align: center; border-bottom: 1px solid #DDD4C8;">
            ...
            <tr>
              <td style="padding: 14px 24px; color: #6B5244; font-size: 14px; font-family: Georgia, serif; border-bottom: 1px solid #EDE8E1; width: 50%;">Employee</td>
              <td style="padding: 14px 24px; color: #1C1410; font-weight: 600; text-align: right; font-size: 14px; font-family: Georgia, serif; border-bottom: 1px solid #EDE8E1;">${displayName}</td>
            </tr>
            <tr>
              <td style="padding: 14px 24px; color: #6B5244; font-size: 14px; font-family: Georgia, serif; border-bottom: 1px solid #EDE8E1;">Test</td>
              <td style="padding: 14px 24px; color: #1C1410; font-weight: 600; text-align: right; font-size: 14px; font-family: Georgia, serif; border-bottom: 1px solid #EDE8E1;">${safeTestTypeName}</td>
            </tr>
            <tr>
    ...
    if (emailError) {
      ...
      return new Response(
        JSON.stringify({
          ...
          notifiedCount: 0,
          emailWarning:
            "Email delivery failed but test recorded",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
